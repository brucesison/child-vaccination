...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $s_m_initial = ucwords(strtolower($s_lname));
    ...
    if (isset($_FILES['staff_pic']) && $_FILES['staff_pic']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($fileTmpPath, $dest_path)) {
            ...
            // Handle the error
            header("Location: ../list_secretary_table.php?status=error_upload");
            exit();
        }
    } else {
        // Handle the error
    ...
    try {
        ...
        header("Location: ../list_secretary_table.php?status=added_successfully");
        exit();
    } catch (PDOException $e) {
        // Handle database errors
        // header("Location: ../list_secretary_table.php?status=error");
