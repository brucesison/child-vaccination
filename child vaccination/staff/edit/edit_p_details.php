...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  try {
    $stmt = $pdo->prepare("UPDATE user_tbl SET u_fname = :u_fname, u_m_name = :u_m_name, u_lname = :u_lname, contact_no = :contact_no, email = :email, barangay = :barangay, street = :street, city = :city, state = :state, zipcode = :zipcode, relationship = :relationship WHERE user_id = :user_id");
    ...
    header("Location: ../view_parent.php?user_id=$parent_id&status=details_updated");
    exit();
  } catch (PDOException $e) {
    header("Location: ../view_parent.php?user_id=$parent_id&status=error");
    exit();
  }
}
