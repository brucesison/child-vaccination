...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_FILES['staff_pic']) && $_FILES['staff_pic']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($fileTmpPath, $dest_path)) {
      ...
      // Handle the error
      header("Location: ../view_admin_doctor.php?staff_id=$staff_id&status=error_upload");
      exit();
    }
  } else {
    // Handle the error
  ...
  try {
    ...
    if ($stmt->execute()) {
      header("Location: ../view_admin_doctor.php?staff_id=$staff_id&status=profile_updated");
      exit();
    } else {
