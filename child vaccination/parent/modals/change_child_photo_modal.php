<div class="modal fade" id="change_child_photo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <form action="edit/edit_child_photo.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="file-upload col-md-12 border border-main p-0 rounded mb-3">
            <input class="col-md-12 p-1 rounded border border-secondary" type="file" name="child_pic" accept="image/*"
              required>
          </div>
        </div>
...
<script>
  ...
  function validateFileUploads() {
    ...
    const alertArea = document.getElementById('alertpic-area');
    ...
    if (child_pic.files.length > 0) {
      ...
      if (!fileType.startsWith('image/')) {
        invalidFiles.push('Child picture must be an image type file.');
      }
      if (fileSize > maxFileSize) {
        invalidFiles.push('Child picture must be less than 2MB.');
      }
    }

    // If there are invalid files, show alert and disable the button
    if (invalidFiles.length > 0) {
      alertArea.innerHTML = '<div class="alert alert-danger">' + invalidFiles.join('<br>') + '</div>';
